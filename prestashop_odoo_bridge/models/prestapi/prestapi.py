...
class PrestaShopWebService(object):
    ...
    def _execute(self, url, method, data=None, files=None, add_headers=None):
        ...
        if not files:
            ...
            r = self.client.request(method, url, files=files,
                                    headers={'User-agent': 'Prestapi: Python Prestashop Library'}, verify=False)
    ...
    def add(self, resource, content, img_filename=None):
        ...
        if img_filename:
            ...
            if isinstance(img_filename, (str)) and 1 <= len(
                    img_filename) <= 255 and "/" not in img_filename and "\000" not in img_filename and '.' in img_filename:
                if self.debug:
